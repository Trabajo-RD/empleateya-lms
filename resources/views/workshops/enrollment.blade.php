                        ...
                        <div class="lg:flex items-center pt-8">
                            <div class="w-40 overflow-hidden mr-6">
                                @isset( $workshop->image )
                                    <img :key="{{ 'image-' . $workshop->id }}" src="{{ Storage::url( $workshop->image->url ) }}" alt="{{ $workshop->title }}" class="w-full object-cover transition duration-300 transform hover:scale-125" />
                                @else
                                    <img :key="{{ 'image-' . $workshop->id }}" class="w-full object-cover transition duration-300 transform hover:scale-125" src="{{ asset('images/courses/default.jpg') }}" alt="{{ $workshop->title }}" >
                                @endisset
                            </div>
                            <div class="lg:flex flex-wrap lg:flex-1 items-center justify-between py-6 pr-6">
                                <div>
                                    <span>
                                        Usted está adquiriendo el <span class="lowercase">{{ __($workshop->type->name) }} </span>
                                    </span>
                                ...
                                <div>
                                    ...
                                    <span class="font-medium text-2xl lg:text-right">{{ $workshop->price->name }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <x-tailwind.cards.card-full>
                        <div class="divide-y">
                            ...
                            <div class="flex justify-between py-4">
                                <span class="text-md font-medium">{{ $workshop->title }}</span>
                                <span class="text-md ml-4">{{ $workshop->price->name }}</span>
                            </div>
                            <div class="py-4">
                                <small>
